#!/usr/bin/env python3
"""
SAC-Gaussian训练脚本 (标准高斯策略SAC)

Baseline: 标准SAC算法使用高斯策略（非扩散策略）
用于证明扩散策略相对于高斯策略的优势
"""
...
# 解析参数
parser = argparse.ArgumentParser(description="SAC-Gaussian训练")
...
parser.add_argument("--output_dir", type=str, default="experiments/baselines/sac_gaussian", help="输出目录")
...
parser.add_argument("--auto_tune_alpha", action="store_true", default=True, help="自动调节温度参数")
...
from baselines.sac_gaussian import SACGaussianAgent


print("="*80)
print("  SAC-Gaussian驾驶策略训练（标准高斯策略）")
print("="*80)
print(f"  并行环境: {args.num_envs}")
print(f"  总步数: {args.total_steps:,}")
print(f"  批次大小: {args.batch_size}")
print(f"  自动调节alpha: {args.auto_tune_alpha}")
print("="*80)
...
def train():
    ...
    run_dir = output_dir / f"sac_gaussian_{timestamp}"
    ...
    agent = SACGaussianAgent(
        ...
        critic_lr=args.lr,
        alpha_lr=args.lr,
        auto_entropy_tuning=args.auto_tune_alpha,
        device=device,
    )
        ...
        if total_steps < args.warmup_steps:
            ...
            with torch.no_grad():
                ...
                for i in range(args.num_envs):
                    action = agent.select_action(obs_np[i], eval_mode=False)
        ...
        if len(replay_buffer) >= args.batch_size and total_steps >= args.warmup_steps:
            ...
            if total_steps % args.log_freq == 0:
                ...
                if total_steps % 1000 == 0:
                    print(f"\n[Step {total_steps:,}] 奖励细节 (SAC-Gaussian):")
